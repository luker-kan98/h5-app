...
def test_rejects_non_http_scheme():
    with pytest.raises(UrlValidationError, match="scheme"):
        validate_h5_url("ftp://example.com", language="en")
...
def test_rejects_localhost():
    with pytest.raises(UrlValidationError, match="internal"):
        validate_h5_url("http://localhost/app", language="en")


def test_rejects_127_0_0_1():
    with pytest.raises(UrlValidationError, match="internal"):
        validate_h5_url("http://127.0.0.1:8080/", language="en")


def test_rejects_private_192():
    with pytest.raises(UrlValidationError, match="internal"):
        validate_h5_url("http://192.168.1.1/", language="en")


def test_rejects_private_10():
    with pytest.raises(UrlValidationError, match="internal"):
        validate_h5_url("http://10.0.0.1/", language="en")


def test_rejects_private_172():
    with pytest.raises(UrlValidationError, match="internal"):
        validate_h5_url("http://172.16.0.1/", language="en")
